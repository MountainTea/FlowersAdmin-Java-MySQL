...
- ### 後台/庫存管理
  ...
  5. 以下表格分析庫存個欄位數字概念。
     ...
     |:--|:--|:--|:--|:--|
     |依據進貨單，真實進貨的數量|依據訂單統計，至今「已下單」數量，但尚未真實出貨|依據訂單統計至今，「已出貨」的數量 | 依據庫存、訂單及進貨單統計至今，<br>最新的庫存 - <br>「已下單」數量(無論出貨否，但排除取消) + <br>「已訂貨」的數量(無論進貨否，但排除取消)|邏輯同「模擬庫存」，只是改成真實 出貨、及真實進貨|
