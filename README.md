...
- ### 後台/庫存管理
  ...
  5. 以下表格分析庫存個欄位數字概念。
     ...
     |:--|:--|:--|:--|:--|
     |依據進貨單統計至今，真實進貨的數量|依據訂單統計至今，「已下單」數量，但尚未真實出貨|依據訂單統計至今，「已出貨」的數量 | 庫存 – 已下單數 + 已訂貨數(上敘下單數，只要不是取消狀態，都會計入) |邏輯同「模擬庫存」，只是改成真實 出貨、及真實進貨|

       因此，只要在庫存介面，將第一個篩選選擇「待補貨」，那就會只撈出「模擬庫存」是負數的部分；  
     例如 產品A 的模擬數量是 -7，意思就是 現在的庫存-有人下單的數量+預計(含真實)進貨的數量= 還少7個，只要再去補一張進貨單，至少進貨7個以上(含)商品 屆時庫存一定>=0。
...
- ### 後台/進貨管理
  ...
  ![後臺進貨圖例](https://github.com/MountainTea/FlowersAdmin-Java-MySQL/blob/main/img/%E7%B5%84%E5%90%884_%E9%80%B2%E8%B2%A8.png "Backstage purchase example")</a>
       
- ### 後台/其他管理介面
  1. 其他如 會員管理、員工管理、商品管理、供應商管理...等，皆較為制式化功能，請直接操作即可。
