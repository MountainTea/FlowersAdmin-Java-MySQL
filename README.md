...
- ### 後台/出貨
  ...
  3. 可在此列表進行「未出貨」訂單之 新增、修改、取消未出貨之訂單；至於「已出貨及取消」的訂單由管理員身分才可操作。
...
- ### 後台/庫存管理
  ...
  5. 以下表格分析庫存個欄位數字概念。
     ...
     |:--|:--|:--|:--|:--|
     |依據進貨單，真實進貨的數量|依據訂單統計，至今「已下單」數量，但尚未真實出貨|依據訂單統計，至今「已出貨」的數量 | 依據庫存、訂單及進貨單統計，至今最新的庫存 - 「已下單」數量(無論出貨否，但排除取消) + 「已訂貨」的數量(無論進貨否，但排除取消)|邏輯同「模擬庫存」，只是改成真實 出貨、及真實進貨|

       因此，只要在庫存介面，將第一個篩選選擇「待捕貨」，那就會只撈出「模擬庫存」是負數的部分；例如 產品A 的模擬數量是 -7，意思就是 現在的庫存-有人下單的數量+預計(含真實)進貨的數量= 還少7個，只要至少進貨7個以上(含) 屆時庫存一定>=0。
